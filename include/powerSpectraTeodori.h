...
class Powermine
{
    ...
    */
    ...
    double integral_mu (double q, double k); ///< Computes the integral in \f$ \mu \f$
    double integral_q (double k);  ///< Computes the integral in \f$ q \f$
    double CurlyP(double a, double k); ///< The complete \f$ \bar{\mathcal{P}} \f$
    
    void writeCurlyP1(); ///< Function for writing data in text file, used then for plotting/analysis 
    void writeCurlyP2(double k); ///< Function for writing data in text file, used then for plotting/analysis
    ...
    // Parameters
    double k_0=10; 
    double A = 100; ///< Normalization of the power spectrum, to be determined by \f$ \sigma_8 \f$  
    double nbins = 32; // seems good also with 32
    double k_max = 1000.0; ///< The maximum \f$ k \f$ used in integration
    double k_min = 0.001;///< The minimum \f$ k \f$ used in integration
    double q_min = 0.001;///< The minimum \f$ q \f$ used in integration
    double q_max = 10000.0;///< The maximum \f$ q \f$ used in integration
